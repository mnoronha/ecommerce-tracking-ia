"""
Pinterest Ads dashboard — server-side attribution + CAPI health.

Aggregates paid orders where utm_source ILIKE 'pinterest%' for revenue
attribution, plus CAPI send statistics. No ad-spend data yet (Pinterest
Ads API sync not implemented), so ROAS/CPA are unavailable.
"""
...
router = APIRouter(prefix="/pinterest-ads", tags=["pinterest_ads"])


# ── Helpers (same pattern as tiktok_ads) ──────────────────────────────────────
...
@router.get("/{pixel_id}/overview", summary="Pinterest Ads — atribuição server-side + saúde CAPI")
async def pinterest_overview(
    pixel_id: str,
    days: int = 30,
    ...
    pinterest_tag  = cli.data[0].get("pinterest_tag_id") or ""
    ...
    capi_rows = (
        sb.table("orders")
        .select("pinterest_sent, pinterest_last_error")
        .eq("client_id", client_uuid)
        .eq("financial_status", "paid")
        .gt("total_price", 0)
    ...
    capi_sent   = sum(1 for r in capi_rows if r.get("pinterest_sent") is True)
    capi_failed = sum(
        1 for r in capi_rows
        if r.get("pinterest_sent") is not True
        and r.get("pinterest_last_error")
        and not str(r["pinterest_last_error"]).startswith("skipped:")
    )

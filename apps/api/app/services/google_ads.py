...
def fetch_adgroup_insights(
    ...
    """Ad-group-level performance for the dashboard (best-effort, [] on failure)."""
    if not all([customer_id, refresh_token,
                settings.GOOGLE_ADS_DEVELOPER_TOKEN,
                settings.GOOGLE_ADS_OAUTH_CLIENT_ID,
                settings.GOOGLE_ADS_OAUTH_CLIENT_SECRET]):
    ...
    try:
        ...
        logger.warning("google_ads fetch_adgroup_insights: %s", exc)
        return []

    if resp.status_code != 200:
        logger.warning("google_ads ad_groups HTTP %s: %s", resp.status_code, resp.text[:300])
        return []
    ...
    logger.info("google_ads: %d ad groups fetched for %s", len(rows), clean_cid)
    return rows
